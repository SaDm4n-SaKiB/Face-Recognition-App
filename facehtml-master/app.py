...
# Save registration data in a temporary variable (this is just an example, usually data is stored in a database)
registered_data = {}
...
def register():
    ...
    # Save the photo in the uploads folder
    ...
    # Save registration data (just an example, usually data is stored in the database)
...
def login():
    ...
    # Save the login photo in the uploads folder
    uploads_folder = os.path.join(os.getcwd(), 'static', 'uploads')
    if not os.path.exists(uploads_folder):
        os.makedirs(uploads_folder)
